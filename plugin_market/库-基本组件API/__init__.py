...
class BasicFunctionLib(PluginAPI):
    ...
    def getScore(self, scoreboardNameToGet: str, targetNameToGet: str) -> int:
        ...
        resultList = self.game_ctrl.sendwscmd(
            "/scoreboard players list %s" % targetNameToGet, True).OutputMessages
        result = {}
        result2 = {}
        for i in resultList:
            Message = i.Message
    ...
    def getPos(self, targetNameToGet: str, timeout: float | int = 1) -> dict:
        ...
        for i in resultList:
            targetName = find_key_from_value(
                self.game_ctrl.players_uuid, i["uniqueId"])
            ...
            position = {"x": float("%.2f" % x), "y": float(
                "%.2f" % y), "z": float("%.2f" % z)}
            dimension = i["dimension"]
            yRot = i["yRot"]
            result[targetName] = {"dimension": dimension,
                                  "position": position, "yRot": yRot}
    ...
    def getTarget(self, sth: str, timeout: bool | int = 5) -> list:
        ...
        result = self.game_ctrl.sendwscmd(
            "/testfor %s" % sth, True, timeout).OutputMessages[0].Parameters
        if result:
            result = result[0]
            return result.split(", ")
        else:
