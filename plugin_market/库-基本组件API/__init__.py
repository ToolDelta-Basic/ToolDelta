...
class BasicFunctionLib(PluginAPI):
    ...
    def getScore(self, scoreboardNameToGet: str, targetNameToGet: str) -> int:
        ...
        resultList = self.game_ctrl.sendwscmd("/scoreboard players list %s" % targetNameToGet, True).OutputMessages
        result = {}
        result2 = {}
        for i in resultList:
            Message = i.Message
    ...
    def getPos(self, targetNameToGet: str, timeout: float | int = 1) -> dict:
        ...
        """
        if (targetNameToGet not in self.game_ctrl.allplayers) and (targetNameToGet != self.game_ctrl.bot_name) and (not targetNameToGet.startswith("@a")):
            raise Exception("Player not found.")
        result = self.game_ctrl.sendwscmd("/querytarget " + targetNameToGet, True, timeout)
        if result.OutputMessages[0].Success == False:
            raise Exception(f"Failed to get the position: {result.OutputMessages[0].Parameters[0]}")
    ...
    def getItem(self, targetName: str, itemName: str, itemSpecialID: int = -1) -> int:
        ...
        if (targetName not in self.game_ctrl.allplayers) and (targetName != self.game_ctrl.bot_name) and (not targetName.startswith("@a")):
            raise Exception("Player not found.")
        result = self.game_ctrl.sendwscmd("/clear %s %s %d 0" % (targetName, itemName, itemSpecialID), True)
    ...
    def getTarget(self, sth: str, timeout: bool | int = 5) -> list:
        ...
        result = self.game_ctrl.sendwscmd("/testfor %s" % sth, True, timeout).OutputMessages[0].Parameters
        if result:
            result = result[0]
            return result.split(", ")
        else:
