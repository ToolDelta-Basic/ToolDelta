...
class ConsoleCommands(Plugin):
    ...
    def on_inject(self):
        self.frame.add_console_cmd_trigger(["/"], "[指令]", "执行MC指令", self.SendMCCmdOnConsoleThread)
        self.frame.add_console_cmd_trigger(["ws/"], "[指令]", "执行WS指令", self.SendWSCmdOnConsole)
        self.frame.add_console_cmd_trigger(["wo/"], "[指令]", "执行控制台权限指令", self.SendWOCmdOnConsole)
        self.frame.add_console_cmd_trigger(["fb", "!"], "[指令]", "执行FB指令 (fb + <fb指令>)", self.SendFBCmdOnConsole)
        self.frame.add_console_cmd_trigger(["list"], None, "查询在线玩家", lambda _: self.print.print_inf(f"在线玩家: " + ", ".join(self.game_ctrl.allplayers)))
        self.frame.add_console_cmd_trigger(["bdump"], "-p <文件路径>", "导入bdx文件", self.SendOriginalFBCommand)
        self.frame.add_console_cmd_trigger(["schematic"], "-p <文件路径>", "导入bdx文件", self.SendOriginalFBCommand)
        self.frame.add_console_cmd_trigger(["plot"], "-p <文件路径>", "导入PNG文件", self.SendOriginalFBCommand)
        self.frame.add_console_cmd_trigger(["mapart"], "-p <文件路径>", "导入图片文件, 且效果更好", self.SendOriginalFBCommand)
