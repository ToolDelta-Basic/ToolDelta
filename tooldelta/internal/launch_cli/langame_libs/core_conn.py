...
class LanGame:
    ...
    def start_connection(self, port: int):
        self.conn = self.make_conn(
            f"ws://127.0.0.1:{port}", self.on_conn, self.on_msg, self.on_clos
        )
        fmts.print_suc("已开启 ToolDelta WS Service")
    ...
    def on_msg(self, ws: WebSocket, msg_raw: str):
        msgdata = msgpack.unpackb(msg_raw, strict_map_key=False)
        try:
    ...
    def handler(self, msg: Message) -> None:
        try:
            match msg.type:
                ...
                case MessageType.MSG_SERVER_PKT_JSON:
                    if self.server_dict_packet_handler is not None:
                        pkID = msg.content["ID"]
                        pk = json.loads(msg.content["Content"])
                        if pkID == constants.PacketIDS.CommandOutput:
                            pkUUID: bytes = uuid.UUID(pk["CommandOrigin"]["UUID"]).bytes
                            if pkUUID in self.command_cbs.keys():
                                self.command_cbs[pkUUID](pk)
                            # else:
                            #    fmts.print_war(f"无效命令返回UUID: {pkUUID} ({self.command_cbs}) {id(self.command_cbs)}")
