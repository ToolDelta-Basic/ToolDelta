...
from ...constants import TOOLDELTA_JAVACONNECT_PLUGIN, TOOLDELTA_PLUGIN_DATA_DIR
...
class Plugin:
    ...
    def __init__(self, frame: "ToolDelta"):
        self.frame = frame
        self.server_ctrl = frame.get_server_control()
...
_PLUGIN_JAVACONNECT_TYPE = TypeVar("_PLUGIN_JAVACONNECT_TYPE")


def add_plugin(plugin: type[_PLUGIN_JAVACONNECT_TYPE]) -> type[_PLUGIN_JAVACONNECT_TYPE]:
    """添加ToolDelta类式JAVACONNECT插件的插件主类
...
def add_plugin_as_api(apiName: str):
    ...
    def _add_plugin_2_api(api_plugin: type[_PLUGIN_JAVACONNECT_TYPE]) -> type[_PLUGIN_JAVACONNECT_TYPE]:
...
def read_plugins(plugin_grp: "PluginGroup") -> None:
    ...
    """
    PLUGIN_PATH = os.path.join("插件文件", TOOLDELTA_JAVACONNECT_PLUGIN)
    sys.path.append(os.path.join("插件文件", TOOLDELTA_JAVACONNECT_PLUGIN))
...
def load_plugin(
    ...
    try:
        if os.path.isfile(
            os.path.join(
                "插件文件", TOOLDELTA_JAVACONNECT_PLUGIN, plugin_dirname, "__init__.py"
            )
        ):
        ...
        plugin_group.self.injected_plugin_loaded_num_plugin_loaded_num += 1
