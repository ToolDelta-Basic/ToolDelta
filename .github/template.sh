...
github_release_url="https://mirror.ghproxy.com/https://github.com/ToolDelta/ToolDelta/releases/download/{{VERSION}}/ToolDelta-linux"
...
# 下载
if sudo curl -o "$app_name" -L "$github_release_url"; then
    echo "下载完成。"
else
...
# 创建符号链接
if sudo ln -s "$install_dir/start.sh" "/usr/local/bin/$shortcut_command"; then
